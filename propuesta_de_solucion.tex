...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{widget-formulas/ejemplo-dash-normal}
...
Se implementará solo en los \textit{widgets} de gráfico y tabla, ya que, utilizan el mismo componente y dependiendo de si es un \textit{widget} de tabla o gráfico muestra ciertos \textit{inputs} en la pestaña de \textbf{Ajustes} como muestra en la figura ... . Además, en el formulario de gráfico, tiene una pestaña extra de \textbf{Configuración de Ejes} (figura ...), en el cual se pueden configurar \textbf{plotlines/plotbands} y configurar los ejes. Además, el formulario de gráfico permite una previsualización con el botón \textbf{Previsualizar} (figura ...).
\iffalse imágenes con los formularios con sus diferencias \fi
\iffalse previsualización gráfico \fi

En los formularios de creación/edición de los \textit{widgets} de gráfico y tabla se agrega una nueva pestaña con el título de 'Fórmulas', el formulario de esta sección constará de una tabla que contendrá lo siguiente:
\begin{itemize}
...
\begin{enumerate}
    ...
    \SubItem{Se itera sobre el \textit{Set} de fechas. Por cada fecha se obtiene el dato de cada variable/fórmula y se ejecuta la función con la fórmula. En el caso en que en una variable no exista la fecha, no se realiza el cálculo y se continua con la siguiente fecha.}
